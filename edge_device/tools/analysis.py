#!/usr/bin/env python3
"""
analyze_results.py - Evaluation Results Analyzer (Final Robust Version)
=====================================================================

Features:
1. Robust Path Handling: Accepts file or directory paths.
2. Resource Analysis: Calculates both PEAK and AVERAGE load per run.
3. CSV Export: Includes ALL timing phases needed for plotting.
"""
...
def load_metrics(path_input: str) -> List[Dict]:
    ...
    if os.path.isfile(path_input):
        ...
        filepath = os.path.join(path_input, "metrics.json")
...
def generate_csv(runs: List[Dict]) -> str:
    ...
    headers = [
        ...
        "peak_mem", "avg_mem", 
        "peak_temp", "avg_temp",
        # [Fixed] Added missing columns required by newplot.py
        "duration_lookup_ms", "duration_parse_ms", 
        "duration_pull_ms", "duration_start_ms", 
        "duration_advertise_ms",
        "error_message"
    ]
    ...
    for run in runs:
        ...
        values = [
            ...
            # Timings (Ensuring all columns exist)
            str(run.get("duration_lookup_ms", "")),
            str(run.get("duration_parse_ms", "")),
            str(run.get("duration_pull_ms", "")),
            str(run.get("duration_start_ms", "")),
            str(run.get("duration_advertise_ms", "")),
